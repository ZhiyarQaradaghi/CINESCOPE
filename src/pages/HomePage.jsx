...
const HomePage = ({ pageType = "home" }) => {
  ...
  // Fetch movies based on page type, search query, filters, and pagination
  useEffect(() => {
    const getMovies = async () => {
      ...
      try {
        ...
        if (searchQuery && searchQuery.trim() !== "") {
          ...
          console.log(`Fetching ${pageType} movies`);
    ...
    >
      ...
      >
        <DialogContent sx={{ p: 0 }}>
          <MovieDetail movie={movieDetails} onClose={handleCloseDialog} />
        </DialogContent>
      </Dialog>
    </Box>
  );
